...
export default function InputForm({ onAddProcess, selectedAlgorithm, isVisualizing }) {
  // Local state for all possible inputs
  const [arrivalTime, setArrivalTime] = useState('');
  const [burstTime, setBurstTime] = useState('');
  const [priority, setPriority] = useState('');
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-4">
      ...
      <button
        type="submit"
        disabled={isVisualizing} // 👈 ADD THIS
        className="w-full bg-blue-600 hover:bg-blue-700 text-white font-bold py-2 px-4 rounded-md transition duration-200 transform hover:scale-102 disabled:opacity-50 disabled:hover:scale-100 disabled:cursor-not-allowed"
      >
        + Add Process
      </button>
    </form>
